...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchWeather = async () => {
      try {
        ...
        setWeatherError('');
        // Replace YOUR_API_KEY with the key you inserted
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      {/* Weather + Weekly Generation (Bottom Section) */}
      <div className="mt-8">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Weather & Weekly Generation</h3>

        {/* Today's Weather quick facts */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Temperature</p>
            <p className="mt-1 text-lg font-semibold text-gray-900">
              {loadingWeather ? '—' : weather?.current?.temp_c != null ? `${weather.current.temp_c}°C` : '—'}
            </p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Wind</p>
            <p className="mt-1 text-lg font-semibold text-gray-900">
              {loadingWeather ? '—' : weather?.current?.wind_kph != null ? `${weather.current.wind_kph} kph` : '—'}
            </p>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <p className="text-xs font-medium text-gray-500 uppercase tracking-wider">Updated (Kolkata)</p>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm text-gray-600">{weather?.location ? `${weather.location.name} Weather, ${weather.location.region}` : 'Bhubaneswar Weather'}</p>
              <p className="text-sm font-medium text-gray-900">
                {loadingWeather ? 'Loading...' : weatherError ? weatherError : `${weather?.current?.temp_c ?? '--'}°C, ${weather?.current?.condition?.text ?? '—'}`}
              </p>
            </div>
          </div>
          {weather?.current && (
            <div className="text-sm text-gray-600">
              Wind: <span className="font-medium text-gray-900">{weather.current.wind_kph} kph</span> · Humidity: <span className="font-medium text-gray-900">{weather.current.humidity}%</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          {weeklyGeneration.map((d, idx) => (
            <div key={d.day} className={`rounded-xl border ${idx === 0 ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'} p-4`}> 
              ...
              <div className="space-y-2">
                <div className="text-sm text-gray-700">
                  <span className="font-semibold">Solar:</span> <span className="font-bold text-gray-900">{d.solar}MW</span>
                </div>
                <div className="text-sm text-gray-500">
                  <span className="">Wind:</span> <span className="font-semibold text-gray-700">{d.wind}MW</span>
                </div>
              </div>
            </div>
          ))}
